...
class pymunkEnv(Env):
    ...
    def reset(self):
        ...
        if self.report_all_data:
            targetLoc, self.tunnel_points = createObstacleField3(**obsKwargs)
            self.convertTunnelPoints()
        else:
            targetLoc = createObstacleField3(**obsKwargs)
...
def createObstacleField3(systemStart, width, height, totalR, space, friction, wallThickness=0.5, report_points=False):
    """
    ...
    if report_points:
        top_wall = [
            [xu0a, yu0a], [xu0b, yu0b], [xu1b, yu1b], [xu2b,yu2b]
        ]
        bottom_wall = [
            [xd0a,yd0a], [xd0b,yd0b], [xd1b,yd1b], [xd2b,yd2b]
        ]
        all_points = [top_wall, bottom_wall]
        return center_end, all_points
