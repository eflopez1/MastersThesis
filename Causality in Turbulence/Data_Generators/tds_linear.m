function data = tds_linear(N,scale)
...
%   more details.
    ...
    for i = 2:N
       ...
       data(i,2) = 0.5*data(i-1,1) + normrnd(0,scale);
       data(i,3) = -0.4*data(i-1,1) + normrnd(0,scale);
       data(i,4) = -0.5*data(i-1,1) + 0.25*sqrt(2)*data(i-1,4) + 0.25*sqrt(2)*data(i-1,5) + normrnd(0,scale);
