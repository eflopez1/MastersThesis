...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, maxNumSteps, R, 
                 ...
                 slidingFriction=0, minSeparation=0):
        """
        ...
        self.single_interior = False # If true, the interior will just be a single object
        ...
        self.R = R
        self.R_pixel = self.convert.Meters2Pixels(R) # This is needed for XYStep function
        ...
        self.targetDistance = targetDistance = R*25
        ...
        # self.state_size = self.numBots*5 + 3 # Also storing the last action and progress made ## self.getOb3()
        self.action_size = self.numBots*2
        # self.action_size = 2 # X and Y position, relative to system center
        force_low, force_high = -1.0, 1.0

        # Since the above defines the observation size, 
        # we should simply define the general observation function that will be used
        # TODO: Create a function which automatically specifies the state_size depending on which observation function is being used.
       
        # In all calls of getting an observation
        self.obFunc = self.getOb
        self.actFunc = self.denseStep
        # self.actFunc = self.XYStep # Only deciding on an x,y position relative to the system to actuate towards
        self.render_target = False # Applies when using XYStep function
        if self.render_target:
            self.rel_pos_loc = (0,0)
        ...
        self.environment_parameters = [['dt:',str(self.dt)],
                                       ...
                                       ['minSeparation',str(self.minSeparation)],
    ...
    def reset(self):
        ...
        kwargs={'space':self.space,
            ...
            'singleInterior':self.single_interior,
        ...
        self.time_on_master_bot = 0 # Only applies if using self.master_bot from singleBotObs   
        self.max_time_on_master_bot = 10 # Only applies if using self.master_bot from singleBotObs  
        self.master_bot = 0 # Defining the master bot in the system
        
        # For the sparse reward!
        self.sparse_reward = False
        if self.sparse_reward:
            self.time_between_rew = 10 # This is in seconds
            self.last_rew_time = 0
        
        # For dual reward
        self.dual_reward = False
        if self.dual_reward:
            self.time_to_escape = 5
            self.current_escape_time = 0 # We will add to this and reset it every escape_time seconds
            self.distance_to_escape = self.R*2
            self.last_systemCenter = self.getSystemCenter()
        ...
        ac = np.array([0]*2) # Take no action. This only applies when using self.getOb3
        self.previousDistance = 0
        observation, _, self.previousDistance, _ = self.obFunc(ac)
        self.last_rotation = 0 # If using self.getOb2, we must rotate the action vector the same amount as the observation!
        return observation
    
    
    def step(self, ac):
        return self.actFunc(ac)
        

    def denseStep(self,ac):
        """
        The original step function used to step the system forward in space
        """
        if np.all(self.distance_storage==0) and (self.time_on_master_bot > self.max_time_on_master_bot): # This only matters when using self.getOb
            new_master_bot = np.random.choice([x for x in range(self.numBots) if x != self.master_bot])
            self.master_bot = new_master_bot # Only matters when using self.getOb2
            self.time_on_master_bot = 0
        else:
            self.time_on_master_bot += self.dt*self.numStepsPerStep
        ...
        # Gather information
        obs, systemCenter, distanceToTarget, sysNormForce = self.obFunc(ac)
        
        if self.sparse_reward: # If using sparse rewards
            if self.last_rew_time >= self.time_between_rew:
                rew = self.calcRew(distanceToTarget, sysNormForce)
                self.previousDistance = distanceToTarget
                self.last_rew_time = 0 
            else:
                rew = 0
                self.last_rew_time += self.dt*self.numStepsPerStep
    ...
    def XYStep(self, ac):
        """
        Receives the x,y position relative to the system's center to actuate towards
        """
        # Get the system center
        systemCenter = self.getSystemCenter(true_pos=True, pixels=True)

        # Get the position we actuate towards for this step
        ac = np.asarray(ac, dtype=float) # Should be the relative X,Y position of the bot
        ac /= np.linalg.norm(ac)
        ac *= self.R_pixel*1.5 # Ensures that the starting point is ALWAYS outside the radius of the system
        rel_pos = systemCenter + ac

        if self.render_target:
            self.rel_pos_loc = pymunk.pygame_util.to_pygame(rel_pos, self.screen) # Converting to pygame coordinates

        # Great, now we need to iterate through each bot and apply a force towards this position
        for i in range(self.numStepsPerStep):
            for index, bot in enumerate(self.bots):
                # Must get this bot's relative position vector to the actuation point
                botPos = np.asarray(bot.body.position)
                vec = rel_pos - botPos # As the bot position changes dynamically, so to does the 
                vec /= np.linalg.norm(vec) # Normalize the vector
                
                # Get the individual force components
                xForce, yForce = self.forceGain*vec

                # Apply a force to this bot in the specified direction
                bot.body.apply_force_at_world_point((xForce, yForce), tuple(botPos)) # Apply the force
        ...
        # Get an observation
        obs, systemCenter, distanceToTarget, _ = self.obFunc(ac) # For this step function, obFunc should be getOb3
    ...
    def getOb(self, ac):
        ...
        if self.dual_reward and self.current_escape_time>self.time_to_escape:
            self.last_systemCenter = systemCenter
    ...
    def getOb2(self, ac):
        ...
        # We roll the observations so the 'first' bot is the first index
        botPos = np.roll(botPos, -bestbotIdx)
        botVel = np.roll(botVel, -bestbotIdx)
        ...
        observation = np.concatenate((botPos.flatten(), botVel.flatten(), botContacts))
        return observation, systemCenter, distanceToTarget, sysNormForce
    ...
    def getOb3(self, ac):
        """
        Positions the forward-most bot to have its observation in the first spot as well.
        Uses a frame stacking technique where the last action and last progress is stored 
        Positions of each bot are relative to the system center.
        """
        
        runTime = [self.timestep/self.maxNumSteps] # Calculated just in case we ever decide to use it
        systemCenter = self.getSystemCenter() # This gives the position relative to the target location
        ...
        botPosCopy = np.copy(botPos)
        botPosCopy -= self.targetLoc
        systemCenter_true = np.mean(botPos, axis=0)
        botPos -= systemCenter_true
        distanceToTarget = np.linalg.norm(systemCenter) # This value is in meters
        
        # Calculate progress for observation
        progress = self.previousDistance - distanceToTarget

        # For energy consumption, but we cannot use this anyways.
        sysNormForce=0
            
        # We are interested in finind the center-most bot at the front
        # mean_y = np.mean(botPos[:,1])
        # order = find_nearest(botPos[:,1], mean_y, 3) # Getting the 3 bots with closest y-position to the average
        # # currentSmallestAngle = np.inf # We consider the bot closest to the target as the one of interest
        # currentClosest = np.inf
        # for botIdx in order:
        #     bot = self.bots[botIdx]
        #     # angle = np.abs(bot.body.angle)
        #     # if angle < currentSmallestAngle:
        #     #     bestbotIdx = botIdx
        #     #     currentSmallestAngle = angle
        #     pos = np.asarray(bot.body.position)
        #     pos = self.convert.Pixels2Meters(pos)
        #     dis = np.linalg.norm(pos-self.targetLoc)
        #     if dis < currentClosest:
        #         bestbotIdx = botIdx
        #         currentClosest = dis

        # Get the bot closest to the target
        distances = np.linalg.norm(botPosCopy, axis=1)
        bestbotIdx = np.argmin(distances)

        # Normalizing observation
        botPos[:,0]=botPos[:,0]/(self.R) #Normalizing X-Coordinate
        botPos[:,1]=botPos[:,1]/(self.R) #Normalizing Y-Coordinate
        # botVel /= self.maxVelocity     #Normalizing Velocity
        # extForces = np.abs(np.concatenate((self.extForcesX, self.extForcesY)))
        # extForces /= self.forceGain
        ...
        observation = np.concatenate((ac, [progress], botPos.flatten(), botVel.flatten(), botContacts))
        return observation, systemCenter, distanceToTarget, sysNormForce


    def getObSingle(self):
        """
        Returns the observation for ONLY the master bot
        """

        sysCenter = self.getSystemCenter()
        disToTarget = np.linalg.norm(sysCenter)

        # Get the master bot
        bot = self.bots[self.master_bot]

        # Get the master bot's position
        botPos = np.asarray(bot.body.position)
        botPos = self.targetLoc - self.convert.Pixels2Meters(botPos)

        # Get the master bot's velocity
        botVel = np.asarray(bot.body.velocity)
        botVel = self.convert.Pixels2Meters(botVel)

        # Get the master bot's contact
        contact = self.botContacts[self.master_bot]

        # Create the observation
        observation = np.concatenate((botPos, botVel, [contact]))

        return observation, sysCenter, disToTarget, 0


    

    def getSystemCenter(self, true_pos = False, pixels=False):
        """
        If true_pos ==True, will return the actual position in space.
        Else returns the position of the system's center relative to the target location
        """
        botPos = np.zeros((self.numBots,2))
        for index, bot in enumerate(self.bots):
            pos = np.asarray(bot.body.position)
            if not pixels: # Pixels==False
                currentBotPos = self.convert.Pixels2Meters(pos)
            else: # Pixels==True
                currentBotPos = pos

            # Define position as relative to the target
            if not true_pos: # True_pos==False
                if pixels: # Pixels==True
                    targetLoc = self.convert.Meters2Pixels(self.targetLoc)
                else: # Pixels==False
                    targetLoc = self.targetLoc
                botPos[index,:] = targetLoc - np.array(currentBotPos)
            else: # True_Pos = True
                botPos[index,:] = np.array(currentBotPos)
    ...
    def calcRew(self, distanceToTarget, sysNormForce=0):

        # Calculate progress
        progress = self.previousDistance - distanceToTarget # Relative to the velocity or speed for arriving at target
        
        rew = progress*2000
        if rew<0: rew = 0 # Do not reward noisy actions! What we do must have significant rewards!
        rew -= sysNormForce*.05
        
        if self.dual_reward:
            sysCenter = self.getSystemCenter()
            dis = np.linalg.norm(sysCenter - self.last_systemCenter)

            if dis > self.distance_to_escape:
                rew += 10
                self.last_systemCenter = sysCenter

        # This reward tracks progress, we so track the progress by tracking reward
        self.distance_storage = np.roll(self.distance_storage, 1)
        self.distance_storage[0] = rew
    ...
    def isDone(self, rew, systemCenter, distanceToTarget):
        ...
        if systemCenter[0] < self.R*2: # If we passed the target, just reward 
            done = True
            rew += 5
            
        if distanceToTarget<self.R/2:
            done=True
            rew += 10
    ...
    def render(self):
        ...
        if self.render_target:
            pygame.draw.circle(self.screen, (255,0,0), self.rel_pos_loc, radius=5)
    ...
    def exportAllData(self):
        ...
        try:
            ...
            self.star_points = "There were no obstacles in this run"
            np.save(self.saveFolder + 'star_coords', self.star_points)
...
def connectBalls(space, theta1, theta2, b1, b2, rest_length, spring_stiffness, 
                 spring_damping, minSeparation, maxSeparation, isBot=[False, False],
                 thetaPlus=[0,0]):
    """
...
def createJamoeba(space, systemCenterLocation, systemRadius, numBots, botMass, 
                  ...
                  inRadius, inMass, inFriction, minSeparation = 0,
                  singleInterior = False, percentInteriorRemove = 0, botCollisionIntStart = 2):
    xCenter = systemCenterLocation[0]
    yCenter = systemCenterLocation[1]
    ...
    for index, body in enumerate(membrane):
        if index < (numBodies-1):
            spring = connectBalls(space, t*index, t*(index+1), body, membrane[index+1], springRL, springK, springB, minSeparation, maxSeparation)
            springs.append(spring)
        else: 
            # Connect last ball to first
            spring = connectBalls(space, t*index, 0, body, membrane[0], springRL, springK, springB, minSeparation, maxSeparation)
            springs.append(spring)
    
    # Create Interiors
    if not singleInterior:
        for index, in_ring in enumerate(gran_per_ring):
            radius = in_rings_radius[index]
            for j in range(in_ring):
                in_theta = j*2*np.pi/in_ring
        ...
        # Creating a single interior particle
        # We use the radius of the largest ring as the 
        radius = in_rings_radius[0]
        interiorParticle = Ball(space, (xCenter, yCenter), radius, inMass, inFriction)
